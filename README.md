...
Enable **Mouse events** on a layer to handle Click, Double click, Right click, Mouse enter, and Mouse leave. The action helper can insert `show_dashboard()`, `toggle_dashboard()`, `open_url("https://example.com/usage")`, `show_context_menu()`, `set(target, property, value)`, `increase(target, property, amount)`, `decrease(target, property, amount)`, `toggle(target, render)`, and `reset(target, property)`. Use `self` for the current layer or select another layer on any root surface; the helper stores its stable ID. Runtime actions can override Render, Visibility, X, Y, Width, Height, and Rotation without changing the saved expressions. Increase and decrease work with numeric properties and accumulate from the current effective value, while `reset` restores the saved expression. URL actions accept only `http` or `https` links, and the context-menu action opens the same menu used by the tray icon at the current pointer position. When both Click and Double click are configured, Click waits for the Windows double-click interval and is suppressed by a double click. The canvas safely previews layer changes without opening dashboards, URLs, or context menus. Tray Icon roots support the same events as one Explorer-owned hit target; their child layers cannot be hit-tested independently.
...
The Claude Code provider additionally exposes the Fable weekly meter while the account reports one: `{claude.fable.percentage}`, `{claude.fable.remaining}`, `{claude.fable.label}` (`Fable`), `{claude.fable.reset.*}`, and `{claude.fable.available}` (1 when a Fable-scoped weekly limit is present, otherwise 0). Fable is a model-scoped slice of the weekly allowance rather than a third independent window, and it counts towards `claude.headline.percentage`. Every provider also exposes `{provider}.five_hour.available` (1 when the provider reported a real five-hour window). The Classic theme appends a narrow Fable column when the meter is available, with the bar on the top row and its `Fable 76% · 1d` label on the bottom row.
...
Date and time expressions use Unix timestamps in seconds. `time.now.unix` and `time.now.milliseconds` provide the live clock; numeric components are available under `time.local.*` and `time.utc.*` as `year`, `month`, `day`, `weekday`, `hour`, `minute`, and `second`. Weekdays use Monday `0` through Sunday `6`. Any Unix timestamp, including provider reset values, accepts `weekday_2`, `weekday_short`, `weekday_long`, `day`, `day_2`, `month`, `month_2`, `month_short`, `month_long`, `year_2`, `year`, `date_short`, `date_long`, `time_short`, `time_seconds`, `time_24`, `time_24_seconds`, `time_12`, `time_12_seconds`, `datetime_short`, `datetime_long`, `iso_date`, `iso_time`, and `iso_datetime`. Prefix a format with `utc_` for UTC; formats without that prefix use the computer's local time zone and the app language for names. For example, `{time.now.unix:time_24}`, `{codex.weekly.reset.unix:date_long}`, and `{codex.weekly.reset.unix - 604800:weekday_2}` show the current time, the weekly reset date, and the weekday on which that seven-day period began. Live clocks redraw at the precision they display: once per second for second-level values and once per minute otherwise.
