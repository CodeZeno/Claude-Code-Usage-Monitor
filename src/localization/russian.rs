...
pub(super) const STRINGS: Strings = Strings {
    ...
    antigravity_setup_menu_item: "Настройка Antigravity...",
    antigravity_setup_title: "Настройка Antigravity",
    antigravity_setup_intro: concat!(
        "Использование Antigravity считывается через собственную функцию строки состояния Antigravity CLI. ",
        "Это приложение не читает учётные данные вашей учётной записи Google и не обращается к Google напрямую.\n\n",
        "1. Откройте Antigravity CLI.\n",
        "2. Выполните команду ниже.\n",
        "3. Используйте Antigravity как обычно — данные об использовании появятся здесь автоматически."
    ),
    antigravity_setup_command_label: "Команда для выполнения (скопирована в буфер обмена):",
    antigravity_setup_disable_label: "Чтобы удалить позже, выполните:",
    antigravity_last_observed_label: "Последнее наблюдение: ",
    antigravity_never_observed: "Использование Antigravity ещё не зафиксировано.",
